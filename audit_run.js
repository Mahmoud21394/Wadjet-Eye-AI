...
// ── BACKEND v7.3: Direct axios login, retry logic ────────────────────────────

// BE-44: signInWithPasswordDirect function exists in supabase.js
check('BE-44-CRIT', 'v7.3: signInWithPasswordDirect() bypasses GoTrueClient TCP hang',
  supabaseConf.includes('function signInWithPasswordDirect') &&
  supabaseConf.includes('axios.post') &&
  supabaseConf.includes('/auth/v1/token?grant_type=password'));

// BE-45: Direct login uses axios with timeout (not GoTrueClient)
check('BE-45-CRIT', 'v7.3: Direct login uses axios with 12s timeout',
  supabaseConf.includes('LOGIN_DIRECT_TIMEOUT_MS') &&
  supabaseConf.includes('12_000'));

// BE-46: Direct login has retry logic (3 attempts)
check('BE-46', 'v7.3: Direct login retries up to 3 times',
  supabaseConf.includes('LOGIN_DIRECT_MAX_RETRIES') &&
  supabaseConf.includes('= 3'));

// BE-47: auth.js uses signInWithPasswordDirect (not createLoginClient)
check('BE-47-CRIT', 'v7.3: auth.js login handler uses signInWithPasswordDirect',
  authRoute.includes('signInWithPasswordDirect') &&
  authRoute.includes('await signInWithPasswordDirect('));

// BE-48: loginError handles axios error codes (ECONNABORTED, ETIMEDOUT)
check('BE-48', 'v7.3: loginError checks axios timeout codes ECONNABORTED/ETIMEDOUT',
  authRoute.includes('ECONNABORTED') && authRoute.includes('ETIMEDOUT'));

// BE-49: diagnostics uses direct axios (not GoTrueClient)
check('BE-49', 'v7.3: diagnostics endpoint uses direct axios (not GoTrueClient)',
  authRoute.includes("ax.get(`${supabaseUrl}/auth/v1/health`") ||
  authRoute.includes("'/auth/v1/health'") &&
  authRoute.includes("require('axios')"));

// FE-37: frontend retry counter capped at 2
check('FE-37', 'v7.3: frontend auto-retry capped at max 2 attempts',
  (() => {
    const patch = require('fs').readFileSync('js/login-secure-patch.js', 'utf8');
    return patch.includes('_autoRetryCount < 2') && patch.includes('_autoRetryCount += 1');
  })());

// FE-38: frontend resets retry counter on success
check('FE-38', 'v7.3: frontend resets _autoRetryCount on login success',
  (() => {
    const patch = require('fs').readFileSync('js/login-secure-patch.js', 'utf8');
    return patch.includes('_autoRetryCount = 0') && patch.includes('_finalizeLogin');
  })());
