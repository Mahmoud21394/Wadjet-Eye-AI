/* ══════════════════════════════════════════════════════════
   EYEbot AI — Main Application Module v17.0
...
async function doLogin() {
  ...
  try {
    ...
    console.info('[EYEbot AI] ✅ Authenticated via real backend as', CURRENT_USER.role, '/', CURRENT_USER.email);
...
function generatePDFReport(type, data) {
  ...
  if (win) {
    win.document.write(`
      ...
      <title>EYEbot AI — ${data.name}</title>
      <style>
      ...
      <h1>👁️ EYEbot AI — Intelligence Report</h1>
      ${content}
      <div class="footer">Generated by EYEbot AI v17.0 · ${new Date().toISOString()} · CONFIDENTIAL</div>
...
function generateCustomerPDFReport(custId) {
  ...
  const win = window.open('', '_blank');
  if (win) {
    win.document.write(`<!DOCTYPE html><html><head><title>EYEbot — ${cust.name}</title>
    ...
    <div class="footer">Generated by EYEbot AI v16.4.7 · ${new Date().toISOString()} · CONFIDENTIAL</div>
  ...
  setTimeout(() => {
    if (format === 'PDF') {
      ...
      if (win) {
        ...
        win.document.write(`<!DOCTYPE html><html><head><title>EYEbot AI — ${tpl.name}</title>
        ...
        <div class="cover"><h1 style="color:#fff;border-bottom-color:#1d6ae5;">👁️ EYEbot AI</h1><h2 style="color:#94a3b8;">${tpl.name}</h2>${tenant?`<p style="color:#64748b;">Tenant: ${tenant}</p>`:''}<p style="color:#64748b;">Generated: ${new Date().toLocaleString()}</p></div>
        ...
        <div class="footer">EYEbot AI v17.0 · ${new Date().toISOString()} · CONFIDENTIAL — DO NOT DISTRIBUTE</div>
        <script>window.print();<\/script></body></html>`);
        win.document.close();
      }
    } else if (format === 'CSV') {
...
function bootSequence() {
  const msgs = [
    { msg:'EYEbot AI v17.0 initialized', type:'info' },
...
function initApp() {
  ...
  if (typeof initLiveData === 'function') {
    initLiveData().catch(err => {
      console.warn('[EYEbot AI] Live data init failed:', err.message);
  ...
  console.log('[EYEbot AI] Platform initialized — v17.0');
  console.log('[EYEbot AI] User:', CURRENT_USER?.name, '|', CURRENT_USER?.role);
  console.log('[EYEbot AI] Backend:', window.THREATPILOT_API_URL);
  console.log('[EYEbot AI] Mode: LIVE DATA + CTI v3.0');
}
