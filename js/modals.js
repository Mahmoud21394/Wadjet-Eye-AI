/* ══════════════════════════════════════════════════════════
   ThreatPilot AI — Modals & Detail Views Module
...
function getCampaignTimeline(c) {
  return [
    ...
    { date:'Dec 5', title:'Campaign Identified', desc:`ThreatPilot AI correlated ${c.iocs} IOCs into campaign cluster.` },
...
function exportPlaybookPDF(id) {
  ...
  <title>ThreatPilot AI — Playbook: ${pb.name}</title>
  <style>
  ...
  <h1>🛡️ ThreatPilot AI — Response Playbook</h1>
  ...
  <div class="footer">Generated by ThreatPilot AI v16.4.7 · ${new Date().toLocaleString()} · CONFIDENTIAL — DO NOT DISTRIBUTE</div>
...
function generateReport(rtId, formatOverride, tenantOverride) {
  ...
  setTimeout(() => {
    if (selectedFmt === 'CSV') {
      ...
      const win = window.open('', '_blank');
      if (win) {
        win.document.write(`<!DOCTYPE html><html><head><title>ThreatPilot — ${rt.name}</title>
        ...
        <div class="cover"><h1 style="color:#fff;border-bottom-color:#3b82f6;">🛡️ ThreatPilot AI</h1><h2 style="color:#94a3b8;">${rt.name}</h2>${tenant?`<p style="color:#64748b;">Tenant: ${tenant}</p>`:''}<p style="color:#64748b;">Generated: ${new Date().toLocaleString()}</p></div>
        ...
        <div class="footer">ThreatPilot AI v16.4.7 · ${new Date().toISOString()} · CONFIDENTIAL</div>
...
function generateReportJSON(rt, tenant) {
  ...
  return JSON.stringify({
    ...
    platform: 'ThreatPilot AI v16.4.7',
    tenant: tenant || 'all',
