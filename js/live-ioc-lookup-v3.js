...
async function _urlhausCheck(value, iocType) {
  ...
  try {
    const body = type==='url' ? { url: value } : type==='ip' ? { host: value } : { host: value };
    ...
    if (d.query_status === 'no_results') return { source:'urlhaus', status:'success', verdict:'CLEAN', urls_count:0 };
