...
async function storeArticles(articles, tenantId) {
  ...
  const records = articles.map(a => ({
    // NOTE: do NOT include 'id' — it is a UUID primary key auto-generated by Postgres.
    // Passing a URL string as the id caused: "invalid input syntax for type uuid"
    tenant_id:        tid,
  ...
  // FIX v11.0: Reduced chunk size from 50 → 20 to avoid Supabase statement-timeout
  // on the free tier (15 s default).  Added 200 ms backpressure between chunks.
  const CHUNK_SIZE = 20;
  ...
  for (let i = 0; i < records.length; i += CHUNK_SIZE) {
    ...
    try {
      ...
      if (error) {
        ...
        const uuidErr      = errMsg.toLowerCase().includes('invalid input syntax for type uuid');

        if (uuidErr) {
          // UUID error means a non-UUID value leaked into a UUID column.
          // Strip any 'id' field from the chunk and retry.
          console.warn('[News] UUID syntax error — stripping id field and retrying');
          const chunkNoId  = chunk.map(r => { const { id, ...rest } = r; return rest; });
          const chunkNoImg = chunkNoId.map(r => { const { image_url, ...rest } = r; return rest; });
          const { error: err2 } = await supabaseClient
            .from('news_articles')
            .upsert(chunkNoImg, { ignoreDuplicates: true });
          if (err2) console.warn('[News] DB upsert (uuid-fix) error:', err2.message);
