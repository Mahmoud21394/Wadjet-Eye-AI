...
// v15.0 — Unified upsert with 3-layer fallback. Root causes fixed:
//
//  UUID error ("invalid input syntax for type uuid: https://...")
//    The 'no img + no constraint' retry branch called upsert() with
//    ignoreDuplicates:true and NO onConflict. Without a named unique
//    index Postgres tries to match on PK (id). If something upstream
//    set an 'id' field to a URL string that UUID parse fails.
//    Fix: NEVER pass an 'id' field. Strip it unconditionally up front.
//    Use onConflict:'external_guid' (single-col) as a safe fallback
//    when the composite index is missing.
//
//  Statement timeout ("canceling statement due to statement timeout")
//    .select('id, created_at') after upsert forces a second round-trip
//    that doubles statement time. Removed. Count conservatively instead.
//
//  "no unique or exclusion constraint" on tenant_id,external_guid
//    Fixed via DB migration (unique index on news_articles). But keep
//    fallback: try onConflict:'external_guid' (simpler) first.
async function storeArticles(articles, tenantId) {
  if (!supabaseClient || !articles.length) return { new: 0, updated: 0 };

  const tid = tenantId || DEFAULT_TENANT;

  // Build records — NEVER include 'id' (UUID PK, auto-generated by Postgres).
  // external_guid must be non-empty: prefer guid, fall back to url, then title hash.
  const records = articles.map(a => {
    const guid = (a.guid || a.link || a.title || Math.random().toString(36)).slice(0, 500);
    // eslint-disable-next-line no-unused-vars
    const { id: _dropId, ...safeBase } = {
      tenant_id:        tid,
      title:            (a.title || '').slice(0, 500),
      url:              (a.link  || '').slice(0, 1000),
      source:           (a.source_name || 'unknown').slice(0, 200),
    ...
    return safeBase; // 'id' is always stripped
  });

  // Chunk size: 15 rows. No .select() after upsert (avoids second DB round-trip).
  // Backpressure: 300 ms between chunks.
  const CHUNK_SIZE = 15;
    ...
    try {
      ...
      if (!error) {
        ...
        if (msg.includes('statement timeout') || msg.includes('canceling statement')) {
          // ── Timeout fallback: wait + retry without image_url ────
          console.warn('[News] Timeout on chunk — retrying without image_url after 2s');
      ...
      const msg = (dbEx.message || '').toLowerCase();
      const isTimeout = msg.includes('statement timeout') || msg.includes('canceling statement') || dbEx.code === '57014';
      if (isTimeout) {
        console.warn('[News] DB exception timeout — retrying without image_url after 2s');
        await new Promise(r => setTimeout(r, 2000));
        try {
          const { error: e2 } = await supabaseClient
            .from('news_articles')
          ...
          if (!e2) { updatedCount += chunk.length; upsertOk = true; }
          else console.warn('[News] Exception-timeout-retry error:', e2.message);
        } catch (e3) {
          console.error('[News] Exception-timeout-retry exception:', e3.message);
        }
      } else {
        console.error('[News] DB chunk exception:', dbEx.message);
      }
    }

    if (!upsertOk && i === 0) {
      // Only log once at start of ingestion to avoid log spam
      console.warn('[News] First chunk failed — subsequent chunks may also fail');
    }
